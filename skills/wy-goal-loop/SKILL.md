...
## 실행 방법
```
/wy-goal-loop 03_tasks/active/T-xxx/       # 과제 폴더 지정
/wy-goal-loop                              # 현재 디렉토리에서 goals.md 탐색
autopilot: /wy-goal-loop으로 goals.md TODO 모두 달성해줘
ralph로 /wy-goal-loop 돌려줘
```
...
- 필요한 데이터/리소스 확인
- 실패 시 rollback 방법 설계
- 관련 스킬 선택 (/wy-model-train, /wy-model-evaluate, /wy-sanity-check 등)
